      ...
      for (var r = 0; r < pRows; r++) {
        for (var c = 0; c < this.channels; c++) {
          ...
          if ((note & 0x80) == 0) {
            ...
            if ((note & 0x01) != 0)
              pattern.note[r][c] = fileData[pDataOffset++];
            if ((note & 0x02) != 0)
              pattern.instrument[r][c] = fileData[pDataOffset++];
  ...
  for (var i = 0; i < this.instrumentCount; i++) {
    ...
    if (instrument.numSamples == 0) {
      ...
      for (var s = 0; s < instrument.numSamples; s++) {
        ...
        sample.fineTune =
          fileData[offset + 13] < 128
            ? fileData[offset + 13]
            : -((fileData[offset + 13] ^ 0xff) + 1);
        sample.loopType =
          sample.loopLength > 0
            ? fileData[offset + 14] & 0x03
            : Sample.LoopType.NONE;
        sample.sampleBits =
          (fileData[offset + 14] & 0x10) == 0
            ? Sample.Bits.FORMAT_8BIT
            : Sample.Bits.FORMAT_16BIT;
        sample.panning = fileData[offset + 15] / 255.0;
        sample.basePeriod = fileData[offset + 16];
        sample.compression =
          fileData[offset + 17] == 0xad
            ? Sample.Compression.ADPCM
            : Sample.Compression.DELTA;
        sample.name = instrument.name; //fileData.substring (offset + 18, offset + 40);

        // Correct sample base period.
        if (sample.basePeriod > 127)
          sample.basePeriod = -(256 - sample.basePeriod);
