...
describe("GBA build file selection", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(Path.join(buildRoot, "src", "main.c"), "int main(void){return 0;}\n");
    await fs.writeFile(Path.join(buildRoot, "src", "startup.s"), ".global _start\n");
    await fs.writeFile(Path.join(buildRoot, "src", "data", "actor_10_interact.s"), ".module actor_10_interact\n");
  });
